...
$kelasMengajar = mysqli_query($con, "SELECT * FROM tb_ajar 

INNER JOIN tb_matkul ON tb_ajar.id_matkul=tb_matkul.id_matkul
INNER JOIN tb_mkelas ON tb_ajar.id_mkelas=tb_mkelas.id_mkelas

INNER JOIN tb_semester ON tb_ajar.id_semester=tb_semester.id_semester
INNER JOIN tb_thajaran ON tb_ajar.id_thajaran=tb_thajaran.id_thajaran
WHERE tb_ajar.id_dosen='$data[id_dosen]' AND tb_ajar.id_ajar='$_GET[pelajaran]'  AND tb_thajaran.status=1  ");
...
<div class="page-inner">
	...
	<div class="row">
		...
		$last_pertemuan = mysqli_query($con, "SELECT * FROM _logabsensi WHERE id_ajar='$_GET[pelajaran]' GROUP BY pertemuan_ke ORDER BY pertemuan_ke DESC LIMIT 1  ");
		...
		<div class="card">
							...
							<div class="item-list">
								<div class="info-user">
									<div class="username">
										...
										$siswa_telah_absen_hari_ini = mysqli_query($con, "SELECT * FROM _logabsensi INNER JOIN tb_mahasiswa ON _logabsensi.id_mahasiswa=tb_mahasiswa.id_mahasiswa WHERE _logabsensi.id_mahasiswa='$s[id_mahasiswa]' AND _logabsensi.tgl_absen='$tgl_hari_ini' AND _logabsensi.id_ajar='$_GET[pelajaran]' AND _logabsensi.ket='' ");
			...
			if (isset($_POST['absen'])) {
				...
				for ($i = 0; $i <= $total; $i++) {
					...
					$cekAbsesnHariIni = mysqli_num_rows(mysqli_query($con, "SELECT * FROM _logabsensi WHERE tgl_absen='$today' AND id_ajar='$pelajaran' AND id_mahasiswa='$id_mahasiswa' "));
